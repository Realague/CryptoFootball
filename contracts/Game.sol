...
contract Game is StorageHelper, ReentrancyGuard {
    ...
    uint[] private staminaRegenPerDay = [70, 80, 100, 120, 140];
    
    TrainingGround[] private trainingGrounds;
    
    bool public trainingOpen = true;
    
    event TrainingDone(address indexed user, bool won, uint rewards, uint xp, bool levelUp, uint playerId);

    event ClaimReward(address indexed user, uint amount);
    
    constructor(address storageAddress) StorageHelper(storageAddress) {
        trainingGrounds.push(TrainingGround(0, 80, 20, 5, 100));
        trainingGrounds.push(TrainingGround(1, 60, 10, 7, 150));
        trainingGrounds.push(TrainingGround(2, 50, 5, 10, 200));
    }
    ...
    function getXpRequireToLvlUp(uint score) internal pure returns (uint) {
        return _getXpRequireToLvlUp(score);
    }
   ...
   function setClaimCooldown(uint _claimCooldown) external onlyOwner {
    ...
    function trainingGround(uint trainingGroundId, uint playerId, bool allTrainingAtOnce) external botPrevention onlyOwnerOf(playerId) {
        require(trainingOpen, "Training is closed");
        Player memory player = _getPlayer(playerId);
        require(player.isAvailable, "");
        uint currentStamina = _getCurrentStamina(player);
        require(currentStamina >= 25, "not enought stamina");
        uint nbTraining = 1;
        if (allTrainingAtOnce) {
            nbTraining = currentStamina / staminaCost;
        }
        player.currentStamina = currentStamina - staminaCost * nbTraining;
        ...
        uint xpGain = tg.xpGain * nbTraining + tg.xpGain * nbTraining * frameBonus[uint(player.frame)] / 100;
        bool won = _train(trainingGroundId, player.score);
        //Do the training
        if (won) {
            xpGain = tg.xpGain * nbTraining + tg.xpGain * nbTraining * frameBonus[uint(player.frame)] / 100;
            uint rewards = (tg.rewards * nbTraining + tg.rewards * nbTraining * frameBonus[uint(player.frame)] / 100) * getFootballTokenPrice();
        ...
        emit TrainingDone(_msgSender(), won, won ? tg.rewards : 0, xpGain, levelUp, playerId);
    }
    ...
    function claimReward() external nonReentrant botPrevention {
        ...
        uint rewards = initialRewards * 10**18 - (getClaimFee() * initialRewards * 10**18 / 100);
